...
vista::RectangularDiagram::RectangularDiagram(QString name, int x, int y, int w, int h, QGraphicsScene* scene, shared_ptr<BoxController> controller)
:
    QGraphicsRectItem(x, y, w, h), Diagram(name, w, h, scene, controller)
{
    ...
    setTransformOriginPoint(box->get_center().x, box->get_center().y);
    setRotation(45);
...
QVariant vista::RectangularDiagram::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant& value)
{
